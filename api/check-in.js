...
// --- ORIGINAL SEATING CONFIGURATION & LOGIC ---
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const [seat1, seat2] = await db.runTransaction(async (t) => {
            ...
            if (next_index + 1 >= TOTAL_AVAILABLE_SEATS) {
                throw new Error("Auditorium is full.");
            }
            
            const s1 = MASTER_SEAT_LIST[next_index];
            const s2 = MASTER_SEAT_LIST[next_index + 1];
            
            t.update(studentDocRef, { 
                checkedIn: true, 
                seat1: s1, 
                seat2: s2, 
                checkInTimestamp: admin.firestore.FieldValue.serverTimestamp() 
            });
            t.set(stateRef, { next_seat_index: next_index + 2 }, { merge: true });
